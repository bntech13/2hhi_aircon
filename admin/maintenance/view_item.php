...
<div class="card">
    <div class="card-header" style="background-color: white; border-bottom: 1px solid #0066cc;">
        <div style="display: inline-block; border: 1px solid #0066cc; border-radius: 10px; padding: 5px 15px; margin-top: 0px;">
           ...
           <i class="nav-icon fas fa-shopping-cart"></i> <b>PURCHASE DETAILS</b>
            </h6>
        </div>
    </div>
    <div class="card-body" id="print_out">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6">
                    <label class="control-label text-info">TR# Code</label>
                    <div></div>
                </div>
                <div class="col-md-6">
            ...
            <table class="table table-striped table-bordered" id="list">
                ...
                <thead>
                    <tr class="text-light bg-navy">
                        ...
                        <th class="text-center py-1 px-2">Total Qty</th>
                ...
                <?php 
                    ...
                    $qry = $conn->query("SELECT p.*,i.brand,i.type,i.hp FROM `po_items` p inner join item_list i on p.item_id = i.id where p.po_id = '{$id}'");
                    while($row = $qry->fetch_assoc()):
                        $total += $row['total'];
                        // Create item description from brand, type, and hp
                        $item_description = $row['brand'] . ' ' . $row['type'] . ', ' . $row['hp'];
                        // Aircon code is not stored in the database for existing items, so we'll leave it blank
                        $aircon_code = '';
                    ?>
                    <tr>
                        <td class="py-1 px-2 text-center">
                            <!-- Empty for existing items -->
                        </td>
                        <td class="py-1 px-2 text-center aircon-code">
                            <?php echo $aircon_code ?>
                        </td>
                    ...
                    <!-- Total Qty Row -->
                    <tr class="bg-light">
                        <td colspan="2" class="text-right py-1 px-2"><strong>Total Quantity:</strong></td>
                        <td class="text-center py-1 px-2"><?php echo number_format($row['quantity'],2) ?></td>
                        <td colspan="4"></td>
                    </tr>
                    <?php endwhile; ?>
                ...
                <tfoot>
                    ...
                    <tr>
                        <th class="text-right py-1 px-2" colspan="6">Tax <?php echo isset($tax_perc) ? $tax_perc : 0 ?>%</th>
...
<table id="clone_list" class="d-none">
    ...
    <!-- Total Qty Row Template -->
    <tr class="total-qty-row">
        <td colspan="2" class="text-right py-1 px-2"><strong>Total Quantity:</strong></td>
        <td class="text-center py-1 px-2 total-qty-value"></td>
        <td colspan="4"></td>
    </tr>
</table>
<script>
